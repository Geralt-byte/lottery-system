...
public class RaffleStrategyController implements IRaffleStrategyService {
    ...
    @Override
    public Response<List<StrategyRuleWeightResponseDTO>> queryRaffleStrategyRuleWeight(@RequestBody StrategyRuleWeightRequestDTO request) {
        try {
            ...
            // 2. 查询用户总抽奖次数
            Integer dayPartakeCount = iRaffleActivityAccountQuotaService.
                    queryRaffleActivityAccountPartakeCount(request.getActivityId(), request.getUserId());

            // 3. 查询规则
            List<StrategyRuleWeightResponseDTO> strategyRuleWeightResponseDTOS = new ArrayList<>();
            List<RuleWeightVO> ruleWeightVOList = iRaffleRule.queryAwardRuleWeightByActivityId(request.getActivityId());
            for (RuleWeightVO ruleWeightVO : ruleWeightVOList) {
                List<StrategyRuleWeightResponseDTO.StrategyAward> strategyAwards = new ArrayList<>();
                ...
                //封装对象
                StrategyRuleWeightResponseDTO strategyRuleWeightResponseDTO = StrategyRuleWeightResponseDTO
                        .builder()
                        .ruleWeightCount(ruleWeightVO.getWeight())
                        .userActivityAccountTotalUseCount(dayPartakeCount)
                        .strategyAwards(strategyAwards)
                        .build();

                strategyRuleWeightResponseDTOS.add(strategyRuleWeightResponseDTO);
            }
            Response<List<StrategyRuleWeightResponseDTO>> response = Response.<List<StrategyRuleWeightResponseDTO>>builder()
                    .code(ResponseCode.SUCCESS.getCode())
                    .info(ResponseCode.SUCCESS.getInfo())
                    .data(strategyRuleWeightResponseDTOS)
                    .build();

            log.info("查询抽奖策略权重规则配置完成 userId:{} activityId：{} response: {}", request.getUserId(), request.getActivityId(), JSON.toJSONString(response));
            return response;
        } catch (Exception e) {
            log.error("查询抽奖策略权重规则失败 userId: {} activityId: {}", request.getUserId(), request.getActivityId(), e);
            return Response.<List<StrategyRuleWeightResponseDTO>>builder()
                    .code(ResponseCode.UN_ERROR.getCode())
                    .info(ResponseCode.UN_ERROR.getInfo())
                    .build();
        }
